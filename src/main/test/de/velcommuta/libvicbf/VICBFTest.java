...
public class VICBFTest {
	...
	public void test_slot_calculation() {
		...
		int v = test.calculateSlot("123".getBytes(), 0);
		assertEquals(v, (short) 9653);
		v = test.calculateSlot("testvicbf".getBytes(), 0);
		assertEquals(v, (short) 8525);
		v = test.calculateSlot("decafbad".getBytes(), 0);
	...
	public void test_increment_calculation() {
		VICBF test = new VICBF(10000, 3);
		byte t = test.calculateIncrement("123".getBytes(), 0);
		assertEquals(t, (byte) 5);
		t = test.calculateIncrement("testvicbf".getBytes(), 0);
		assertEquals(t, (byte) 7);
		t = test.calculateIncrement("decafbad".getBytes(), 0);
		assertEquals(t, (byte) 5);
	}
